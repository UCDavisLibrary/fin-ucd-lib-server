...
// import { AnnotationEditorType } from './util'
import { createPromiseCapability, PermissionFlag } from './util';
import {
  ...
  PixelsPerInch,
  // version,
} from './display_utils'; //'pdfjs-dist/lib'; // "pdfjs-lib";
import {
  DEFAULT_SCALE,
  DEFAULT_SCALE_DELTA,
  DEFAULT_SCALE_VALUE,
...
class PDFViewer {
  ...
  constructor(options) {
    ...
    this.textLayerMode = options.textLayerMode ? options.textLayerMode : TextLayerMode.ENABLE;
    // this.annotationMode =
    //   options.annotationMode ? options.annotationMode : AnnotationMode.ENABLE_FORMS;
    // this.annotationEditorMode =
    //   options.annotationEditorMode ? options.annotationEditorMode : AnnotationEditorType.NONE;
  ...
  get pageViewsReady() {
    ...
    return this._pages.every(function (pageView) {
      return pageView ? pageView.pdfPage : null;
    });
  }
  ...
  _setCurrentPageNumber(val, resetCurrentPageView = false) {
    ...
    this.eventBus.dispatch("pagechanging", {
      source: this,
      pageNumber: val,
      pageLabel: this._pageLabels && this._pageLabels[val - 1] ? this._pageLabels[val - 1] : null,
      previous,
    });
  ...
  get currentPageLabel() {
    return this._pageLabels && this._pageLabels[this._currentPageNumber - 1] ? this._pageLabels[this._currentPageNumber - 1] : null;
  }
  ...
  setPageLabels(labels) {
    ...
    for (let i = 0, ii = this._pages.length; i < ii; i++) {
      this._pages[i].setPageLabel(
        this._pageLabels && this._pageLabels[i] ? this._pageLabels[i] : null
      );
    }
  }
  ...
  async ensurePdfPageLoaded(pageView) {
    ...
    try {
      ...
      if (!this.linkService._cachedPageNumber || !this.linkService._cachedPageNumber(pdfPage.ref)) {
  ...
  getScrollAhead(visible) {
    if (visible.first && visible.first.id === 1) {
      return true;
    } else if (visible.last && visible.last.id === this.pagesCount) {
      return false;
    }
  ...
  //   pageDiv,
  //   pdfPage,
  //   annotationStorage = this.pdfDocument ? this.pdfDocument.annotationStorage : null,
  //   imageResourcesPath = "",
  //   renderForms = true,
  //   l10n = NullL10n,
  //   enableScripting = this.enableScripting,
  //   hasJSActionsPromise = this.pdfDocument ? this.pdfDocument.hasJSActions() : null,
  //   mouseState = this._scriptingManager ? this._scriptingManager.mouseState : null,
  //   fieldObjectsPromise = this.pdfDocument ? this.pdfDocument.getFieldObjects() : null,
